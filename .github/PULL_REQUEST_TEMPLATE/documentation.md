---
name: Documentation
about: Describe this issue template's purpose here.
title: ''
labels: ''
assignees: ''

---

---
name: 📚 Documentation Pull Request
about: Use this template for documentation updates
title: "[DOCS] "
labels: ["documentation"]
assignees: []
---

## 📚 Documentation Pull Request

### 📋 Description
<!-- Provide a clear and concise description of the documentation changes -->

### 🎯 Type of Change
- [ ] Documentation update
- [ ] README update
- [ ] API documentation
- [ ] Code comments
- [ ] User guide
- [ ] Developer guide

### 🔍 What does this PR do?
<!-- Describe the documentation changes in detail -->

### 📖 What documentation was updated?
<!-- List the specific files or sections that were updated -->

- [ ] README.md
- [ ] API documentation
- [ ] Code comments
- [ ] User guides
- [ ] Developer guides
- [ ] Other (please specify)

### 🧪 How has this been tested?
<!-- Please describe how you verified the documentation changes -->

- [ ] Grammar and spelling checked
- [ ] Links verified
- [ ] Code examples tested
- [ ] Screenshots updated
- [ ] Other (please describe)

### 📸 Screenshots (if applicable)
<!-- Add screenshots if the documentation includes UI changes -->

### 📝 Checklist
- [ ] My documentation follows the project's style guidelines
- [ ] I have performed a self-review of my documentation
- [ ] I have verified all links are working
- [ ] I have checked grammar and spelling
- [ ] I have tested any code examples provided
- [ ] I have updated any related documentation
- [ ] The documentation is clear and easy to understand

### 🔗 Related Issues
<!-- Link to any related issues -->
Addresses #(issue number)

### 📚 Additional Notes
<!-- Add any other context about the documentation changes -->
